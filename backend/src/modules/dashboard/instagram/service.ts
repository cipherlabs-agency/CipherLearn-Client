...
export class InstagramService {
     ...
     * Matches comment text against active rules and sends DM.
     */
    async processCommentWebhook(payload: {
        ...
        const { igUserId, mediaId, commentId, commentText, commenterId, commenterUsername } =
            payload;
        ...
        if (!account) {
            logger.warn(`Webhook: No account found for IG user ${igUserId}`);
            return;
        }
        ...
        for (const rule of rules) {
            if (upperComment.includes(rule.triggerKeyword)) {
                try {
                    ...
                    logger.info(
                        `DM sent to ${commenterUsername || commenterId} via rule #${rule.id}`
                    );
                } catch (err: any) {
                    const isRateLimit =
                    ...
                    logger.error(
                        `DM failed for rule #${rule.id}: ${err.message}`
                    );
                }
            }
        }
